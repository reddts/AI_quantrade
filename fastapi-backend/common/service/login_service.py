...
from config.constant import CommonConstant, MenuConstant, PermConstant
from config.enums import RedisInitKeyConfig
from config.env import AppConfig, JwtConfig
from config.get_db import get_db
from exceptions.exception import LoginException, AuthException, ServiceException
from common.dao.login_dao import login_by_account,check_user_exists
...
from common.entity.vo.login_vo import MenuTreeModel, MetaModel, RouterModel, SmsCode, UserLogin, UserRegister
from common.entity.vo.user_vo import AddUserModel, CurrentUserModel, ResetUserModel, TokenData, UserInfoModel
...
class CustomOAuth2PasswordRequestForm(OAuth2PasswordRequestForm):
    ...
    def __init__(
        ...
        code: str = Form(...),
        uuid: str = Form(...),
        login_info: Optional[str] = Form(None),
        # 其他参数为兼容 OAuth2 可选加上
    ...
    async def get_current_user(
        ...
        if token == redis_token:
            ...
            if 1 in role_id_list:
                ...
                user_perm_info = query_user.get('user_perm_info') or []
                permissions = list({row.perms for row in user_menu_info + user_perm_info if row.perms})
    ...
    async def register_user_services(cls, request: Request, query_db: AsyncSession, user_register: UserRegister):
        ...
        if user_register.password == user_register.confirm_password:
            if register_enabled:
                ...
                # 检查用户名和电话号码是否已存在
                is_user = await check_user_exists(query_db, user_register.username,user_register.phonenumber)
                if is_user:
                    raise ServiceException(message='用户名或电话号码已被注册')                
