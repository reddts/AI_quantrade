...
from common.entity.vo.user_vo import UserInfoModel, UserRolePageQueryModel
        ...
        if not await cls.check_role_name_unique_services(query_db, page_object):
            ...
            try:
                ...
                if page_object.perm_ids:
                    for perm in page_object.perm_ids:
                        await RoleDao.add_role_perm_dao(query_db, RolePermModel(roleId=role_id, permId=perm))
        ...
        if role_info:
            ...
            try:
                ...
                if page_object.type != 'status':
                    ...
                    # 删除角色应用权限
                    await RoleDao.delete_role_perm_dao(query_db, RolePermModel(roleId=page_object.role_id))
                    #增加新的角色管理端菜单权限
                    if page_object.menu_ids:
                        for menu in page_object.menu_ids:
                    ...
                    if page_object.perm_ids:
                        for perm in page_object.perm_ids:
                            await RoleDao.add_role_perm_dao(
                                query_db, RolePermModel(roleId=page_object.role_id, permId=perm)
                            )
                await query_db.commit()
        ...
        if page_object.role_ids:
            ...
            try:
                for role_id in role_id_list:
                    ...
                    # 删除角色权限关联表
                    await RoleDao.delete_role_perm_dao(query_db, RolePermModel(**role_id_dict))
