...
class SysUser(Base):
    ...
    birthday = Column(DateTime(timezone=True), default=datetime(1980, 10, 10, tzinfo=timezone.utc), comment='生日')
    isvip = Column(Integer, default=0, comment='是否为VIP（0否 1是）')  
    vip_expiration =Column(Integer, default='0', comment='VIP过期时间')
    theme = Column(String(50), default='', comment='主题')
    signature = Column(String(100), default='', comment='个性签名')
