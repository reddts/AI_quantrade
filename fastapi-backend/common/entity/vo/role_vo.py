...
class RoleModel(BaseModel):
    ...
    perm_check_strictly: Optional[Union[int, bool]] = Field(default=None, description='权限树选择项是否关联显示')
    ...
    @field_validator('menu_check_strictly', 'perm_check_strictly', 'dept_check_strictly')
    @classmethod
...
class RolePermQueryModel(BaseModel):
    """
    角色权限查询模型
    """
    ...
    perms: List = Field(default=[], description='权限信息')
    checked_keys: List[int] = Field(default=[], description='已选择的权限ID信息')
...
class AddRoleModel(RoleModel):
    ...
    perm_ids: List = Field(default=[], description='权限ID信息')
