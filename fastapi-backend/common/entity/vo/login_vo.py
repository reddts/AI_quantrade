...
from common.entity.vo.menu_vo import MenuModel
...
class UserRegister(BaseModel):
    ...
    password: str = Field(description='用户密码')
    phonenumber: Optional[str] = Field(default=None, description='用户手机号')
    ...
    @model_validator(mode='after')
    def check_username(self) -> 'UserRegister':
        # 1. 敏感词检测（忽略大小写）
        sensitive_keywords = ['admin', 'root', 'administrator', '管理员']
        if any(word in self.username.lower() for word in sensitive_keywords):
            raise ModelValidatorException(message='用户名不能包含敏感词，如：admin、root、管理员等')

        # 2. 特殊字符检测
        pattern = r"""^[^<>"'|\\]+$"""
        if not re.match(pattern, self.username):
            raise ModelValidatorException(message='用户名不能包含非法字符：< > " \' \\ |')

        # 3. 长度检测
        if len(self.username) < 4:
            raise ModelValidatorException(message='用户名长度不能少于4个字符')
...
class Token(BaseModel):
    ...
    refresh_token: Optional[str] = Field(default=None, description='刷新token')
