...
from common.entity.do.role_do import SysRole, SysRoleMenu, SysRoleDept, SysRolePerm
from common.entity.do.user_do import SysUser, SysUserRole
from common.entity.vo.role_vo import RoleDeptModel, RoleMenuModel, RoleModel, RolePageQueryModel,RolePermModel
...
class RoleDao:
    ...
    @classmethod
    async def get_role_perm_dao(cls, db: AsyncSession, role: RoleModel):
        """
        根据角色id获取角色权限关联列表信息
        ...
        role_perm_query_all = (
            (
                await db.execute(
                    select(SysPerm)
                    .join(SysRolePerm, SysRolePerm.perm_id == SysPerm.perm_id)
                    .where(
                        SysRolePerm.role_id == role.role_id,
                        ~SysPerm.perm_id.in_(
                            select(SysPerm.parent_id)
                            .select_from(SysPerm)
                            .join(
                                SysRolePerm,
                                and_(SysRolePerm.perm_id == SysPerm.perm_id, SysRolePerm.role_id == role.role_id),
                            )
                        )
                        if role.perm_check_strictly
                        else True,
                    )
                    .order_by(SysPerm.parent_id, SysPerm.order_num)
                )
            )
            .scalars()
            .all()
        ...
        return role_perm_query_all

    @classmethod
    async def add_role_perm_dao(cls, db: AsyncSession, role_perm: RolePermModel):
        """
        新增角色权限关联信息数据库操作

        :param db: orm对象
        :param role_perm: 用户角色权限关联对象
        :return:
        """
        db_role_perm = SysRolePerm(**role_perm.model_dump())
        db.add(db_role_perm)

    @classmethod
    async def delete_role_perm_dao(cls, db: AsyncSession, role_perm: RolePermModel):
        """
        删除角色权限关联信息数据库操作

        :param db: orm对象
        :param role_perm: 角色权限关联对象
        :return:
        """
        await db.execute(delete(SysRolePerm).where(SysRolePerm.role_id.in_([role_perm.role_id])))
