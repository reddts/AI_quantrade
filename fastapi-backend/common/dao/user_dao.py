...
from common.entity.do.menu_do import SysMenu
from common.entity.do.perm_do import SysPerm
from common.entity.do.post_do import SysPost
from common.entity.do.role_do import SysRole, SysRoleDept, SysRoleMenu,SysRolePerm  # noqa: F401
from common.entity.do.user_do import SysUser, SysUserPost, SysUserRole
...
class UserDao:
    ...
    async def get_user_by_id(cls, db: AsyncSession, user_id: int):
        ...
        """
        return await cls._get_user_info(db, user_id, filter_status=True)
    ...
    async def get_user_detail_by_id(cls, db: AsyncSession, user_id: int):
        ...
        :return: 当前user_id的用户信息对象
        """
        return await cls._get_user_info(db, user_id, filter_status=False)
    
    @classmethod
    async def _get_user_info(cls, db: AsyncSession, user_id: int, filter_status: bool = True) -> dict:
        """
        获取用户完整信息
        :param db: orm对象
        :param user_id: 用户id
        :param filter_status: 是否过滤 status=0 和 del_flag=0
        :return: 用户相关信息字典
        """
        filters = [SysUser.user_id == user_id]
        if filter_status:
            filters.extend([SysUser.status == '0', SysUser.del_flag == '0'])

        user_basic_info = await cls._get_user_basic_info(db, filters)
        user_dept_info = await cls._get_user_dept_info(db, filters)
        user_role_info = await cls._get_user_role_info(db, filters)
        user_post_info = await cls._get_user_post_info(db, filters)
        user_menu_info = await cls._get_user_menu_info(db, filters, user_role_info)
        user_perm_info = await cls._get_user_perm_info(db, filters, user_role_info)
        ...
        return {
            ...
            "user_perm_info": user_perm_info,
        }

    @staticmethod
    async def _get_user_basic_info(db: AsyncSession, filters):
        result = await db.execute(select(SysUser).where(*filters).distinct())
        return result.scalars().first()

    @staticmethod
    async def _get_user_dept_info(db: AsyncSession, filters):
    ...
    @staticmethod
    async def _get_user_role_info(db: AsyncSession, filters):
        result = await db.execute(
            select(SysRole)
            .select_from(SysUser)
            .where(*filters)
    ...
    @staticmethod
    async def _get_user_post_info(db: AsyncSession, filters):
        result = await db.execute(
            select(SysPost)
            .select_from(SysUser)
            .where(*filters)
    ...
    @staticmethod
    async def _get_user_menu_info(db: AsyncSession, filters, user_roles):
        role_id_list = [r.role_id for r in user_roles]
        if 1 in role_id_list:
            result = await db.execute(select(SysMenu).where(SysMenu.status == '0').distinct())
            return result.scalars().all()
        else:
            result = await db.execute(
                select(SysMenu)
    ...
    @staticmethod
    async def _get_user_perm_info(db: AsyncSession, filters, user_roles):
        role_id_list = [r.role_id for r in user_roles]
        if 1 in role_id_list:
            result = await db.execute(select(SysPerm).where(SysPerm.status == '0').distinct())
            return result.scalars().all()
        else:
            result = await db.execute(
                select(SysPerm)
...
#根据phonenumber查询数据库中是否存在相同的号码
    @classmethod
    async def get_user_by_phonenumber(cls, db: AsyncSession, phonenumber: str):
        """
        根据手机号获取用户信息
        ...
        query_user_info = (
            (
                await db.execute(
                    select(SysUser)
                    .where(SysUser.del_flag == '0', SysUser.phonenumber == phonenumber)
                    .order_by(desc(SysUser.create_time))
                    .distinct()
                )
            )
